...
def get_graph_outputs(graph) -> dict:
    """
    Generates output matrices of the graph's node attributes with a varying size
    """
    node_pos = graph.positions_vector
    node_degrees = np.asarray(graph.num_node_neighbours) #function in PolyGraph is defined to return a list
    node_types = np.asarray(graph.node_types) #function in PolyGraph is defined to return a list
    stacked_adj_matrix = graph.squared_stacked_adj_matrix()
    # num_nodes = len(self)
    # stacked_matrix = np.zeros((num_nodes,  num_nodes))
    ...
    return {
        ...
        "stacked_adj_matrix": stacked_adj_matrix
    }
