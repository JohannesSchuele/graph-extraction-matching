...
class PolyGraph(nx.Graph):
    ...
    @property
    def adj_matrix_square(self) -> np.ndarray:
    ...
    def coeff2_matrix(self) -> np.ndarray:
        return np.hstack((
            np.zeros(self.positions_vector.shape),
            nx.attr_matrix(self, edge_attr="deg2")[0]
        ))
    ...
    def stacked_adj_matrix(self) -> np.ndarray:
        ...
        stacked_matrix[2, :, :] = self.coeff2_matrix
        stacked_matrix[3, :, :] = self.coeff3_matrix

        return stacked_matrix


    def squared_stacked_adj_matrix(self) -> np.ndarray:
        num_nodes = len(self)
        stacked_matrix = np.zeros((4, num_nodes, num_nodes))
        stacked_matrix[0, :, :] = nx.convert_matrix.to_numpy_array(self)
        stacked_matrix[1, :, :] = nx.attr_matrix(self, edge_attr="length")[0]
        stacked_matrix[2, :, :] = nx.attr_matrix(self, edge_attr="deg2")[0]
        stacked_matrix[3, :, :] = nx.attr_matrix(self, edge_attr="deg3")[0]
        return stacked_matrix
