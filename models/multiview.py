...
class CameraTorch:
    def __init__(self, R_cv, t_cv, K, image_size, dist=None, device='cpu', name=""):
        # transformation can be stated in words as:
        # “first scale, then rotate, then translate”.

        self.device = device
        # following from open_cv coordinate to open_gl, respective natural coordinate system
        # Rotate camera coordinate system!
        # Caution!
        # Image coordinate system follow the pytorch3d image coordinate system,
        # x-axis from left to right, and y-axis from bottom to top!!
        #t=t_cv
        R_z_90 = torch.eye(3, device=self.device)[None]
        R_z_90[0, 0, 1] = -1.0
        R_z_90[0, 1, 0] = 1.0
        R_z_90[0, 0, 0] = 0.0
        R_z_90[0, 1, 1] = 0.0
        R_trafo = R_z_90
        # translation is rotated in a pytorch3d camera, where in contrast a opencv camera is translated first!!
        # compare definition of the extrinsic operations and M matrices!!
        t = torch.matmul(t_cv, R_trafo)[0]
        R_cv = R_trafo[0].t()[None] @ R_cv

        self.R = R_cv.clone().reshape(3, 3)
        assert self.R.shape == (3, 3)
    ...
    @property
    def projection(self):
        return torch.matmul(self.K, self.extrinsics)

    @property
    def extrinsics(self):
        # translation vector are stacked in the negative direction for the pytorch renderer function (See pp-slides!)
        # here in rotation and translation are separate and in positive orientation!
...
def project_3d_points_to_image_plane_without_distortion(proj_matrix, points_3d, image_size, convert_back_to_euclidean=True):
    ...
    if isinstance(proj_matrix, np.ndarray) and isinstance(points_3d, np.ndarray):
        ...
        if convert_back_to_euclidean:
            ...
            result[:, 1] = image_size[1] - result[:, 1]  # coordinate system with y-axis from top to bottom, as the convention in pytroch3d
        return result
    elif torch.is_tensor(proj_matrix) and torch.is_tensor(points_3d):
        result = euclidean_to_homogeneous(points_3d) @ proj_matrix.t()  # coordinate system with y-axis from botttom to top
        if convert_back_to_euclidean:
            result = homogeneous_to_euclidean(result)
            result[:, 1] = image_size[1] - result[:, 1] # coordinate system with y-axis from top to bottom, as the convention in pytroch3d
        return result
...
def calc_reprojection_error_matrix(keypoints_3d, keypoints_2d_list, proj_matricies, image_size):
    reprojection_error_matrix = []
    if keypoints_2d_list.shape.__len__() == 2:
        keypoints_2d, proj_matrix = keypoints_2d_list, proj_matricies
        keypoints_2d_projected = project_3d_points_to_image_plane_without_distortion(proj_matrix, keypoints_3d, image_size)
        reprojection_residual = (keypoints_2d - keypoints_2d_projected) ** 2
        loss = reprojection_residual.mean()
        return loss, reprojection_residual
