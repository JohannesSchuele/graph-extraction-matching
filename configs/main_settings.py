...
raster_settings_soft = RasterizationSettings(
    image_size= training_generator.process_image_dim,
...
image_size_cv = torch.tensor(([training_generator.process_image_dim, training_generator.process_image_dim]))[None].type(dtype=torch.float32)
camera = cameras_from_opencv_projection(
    R_cv,
    t_cv,
    camera_matrix_cv,
    image_size_cv)
camera.focal_length = camera.focal_length.clone()*(training_generator.process_image_dim/128)
...
target_cameras = PerspectiveCameras(device=device, R=R,
                                            T=T, focal_length=camera.focal_length, principal_point=(-1.6, -1.6))
R_cv, t_cv, K_cv = opencv_from_cameras_projection(cameras=target_cameras, image_size=torch.tensor([training_generator.process_image_dim, training_generator.process_image_dim])[None])
####
...
camera_init = camera
camera_init.principal_point = ((0.5, 0.5),)
RenderersCollection = InitRenderers(camera_texture=camera_init, image_size=training_generator.process_image_dim, camera_view=None,
